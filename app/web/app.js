...
async function refreshSeries(){
  ...
  try{
    ...
    const apiCalls=[api(`/api/monitor/series?${query}`),api(`/api/monitor/events?deviceId=${encodeURIComponent(deviceId)}&limit=500`)];
    const results=await Promise.allSettled(apiCalls);
    if(requestId!==state.trendRequestId||deviceId!==state.selectedDeviceId)return;
    if(results[0].status==="fulfilled")state.series=results[0].value;
...
function drawTrendChart(){
  ...
  const series=selectedTrendRows(),bounds=trendBounds(series)||trendEventBounds();$("chartEmpty").classList.toggle("hidden",Boolean(bounds));if(!bounds)return;
