...
面向 YLDQ 6.0 下位机的实时监控、可视化、控制和配置软件。通信协议固定为 Modbus V9.1（协议字 `0x0901`），串口直接传输标准 Modbus RTU ADU，不使用 SLIP，也不兼容旧版寄存器地址。
...
- 运行区安全控制（HR800–807）及阀门动作保护状态（HR817–820）
- 配置暂存、回读、提交和放弃事务
- 阀门配置使用上、左、右三路“空闲位置”，不再提供“初始路由”和“上电初始位置”
...
进入“设备管理”配置串口和从站地址，选择设备后点击“启动监控”。首次收到数据时会校验 FC04 地址 0 必须等于 `0x0901`。
