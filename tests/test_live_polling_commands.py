...
class LivePollingCommandsTests(unittest.TestCase):
    ...
    def test_configuration_blocks_are_manual_refresh_only(self) -> None:
        ...
        self.assertEqual({item["address"] for item in config_commands}, {100, 163, 175, 200, 220, 300, 400, 500, 600, 700, 720})
        ...
        self.assertEqual(valve_command["count"], 16)
        self.assertIn("holding.valve_route.route_cycle_days", valve_command["catalogItemIds"])
        self.assertIn("holding.valve_route.valve_cooling_hours", valve_command["catalogItemIds"])
        self.assertIn("holding.valve_route.idle_position_right", valve_command["catalogItemIds"])
        self.assertNotIn("holding.valve_route.initial_route", valve_command["catalogItemIds"])
        threshold_command = next(item for item in config_commands if item["address"] == 163)
